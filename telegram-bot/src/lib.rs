...
impl Api {
    ...
    fn request<T: Decodable>(client: &Client,
                             url: &Url,
                             method: &str,
                             p: Params,
                             typ: RequestType)
    ...
    fn multipart_request<T: Decodable>(url: &Url,
                                       method: &str,
                                       p: Params,
                                       file: SendPath)
                                       -> Result<T> {
        ...
        match try!(json::decode(&body)) {
            ...
            Response { ok: false, description: Some(desc), ..} => Err(Error::Api(desc)),
            // If response is "ok": Return the result.
            Response { ok: true, result: Some(res), ..} => Ok(res),
    ...
    fn post_request<T: Decodable>(client: &Client,
                                  url: &Url,
                                  method: &str,
                                  p: Params)
                                  -> Result<T> {
